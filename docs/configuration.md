...
| `nss` | `--nss` | Name-service (NSS) modules to stage for glibc name lookups: `files`, `dns`, or `none` (list). Fewer modules trim CVE surface. `none` stages no modules and no `nsswitch.conf`. Default: `files` and `dns`. |
