...
- **Don't silently break the v1.0 contract** (`COMPATIBILITY.md`). The stable surfaces are the CLI flags (names/shorts/defaults/`multiple`/enum values), `scratchsmith.toml` keys, `--format json` fields, and exit codes. In a **minor/patch** these change **additively only**. Removing/renaming a flag or key, dropping a short, changing a default, making a flag newly required, tightening validation, or removing an enum value is a **major** change — first **deprecate** (keep it working, warn on **stderr**, exit code unchanged) for at least the rest of the major line, then remove it in the next major. The **`cli_surface`** golden test flags any surface change; a needed `BLESS=1` regen is the cue to check it isn't breaking. The Rust **library API is not a contract** (`cargo-semver-checks` runs informationally only).
...
Per-release state, open-PR status, and one-off task notes belong in the maintainer's scratch notes, not this file. No file-by-file dumps or generated API docs (link instead). Don't restate things an agent learns in-session.
