...
- **IMPORTANT: Every change is a PR.** Never commit to `main`. Squash-merge, use a **conventional PR title**, and **resolve every review thread** (the ruleset blocks merge otherwise). Every **user-facing** change needs a `.changeset/*.md` fragment. An internal-only PR (CI, **refactor**, tests, non-user-facing docs) needs none, and gets the `no-changelog` label instead. A refactor that touches `src/` still counts as internal-only, which is exactly what that label is for.
