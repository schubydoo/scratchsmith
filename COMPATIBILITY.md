...
- **CLI surface** — the names and meaning of `pack` / `lint` / `doctor` / `index` / `graph` /
  `diff` / `unpack` flags and positionals, and the top-level `--completions`, plus their short
  aliases, their defaults, whether they take a value or are repeatable, and the accepted **enum
  values** (e.g. `--sbom-format cyclonedx-json`, `--scan-fail-on high`).
